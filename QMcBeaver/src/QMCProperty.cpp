...
void QMCProperty::operator = ( const QMCProperty & rhs )
{
  for(int i=0; i<DCL; i++)
    {
      DeCorr[i] = rhs.DeCorr[i];
      DeCorr_flags[i] = rhs.DeCorr_flags[i];
      DeCorr_sample[i] = rhs.DeCorr_sample[i];
      DeCorr_weight[i] = rhs.DeCorr_weight[i];
    }
...
void QMCProperty::readXML(istream& strm)
{
  string temp;
