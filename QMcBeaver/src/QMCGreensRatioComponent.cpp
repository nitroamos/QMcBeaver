  ...
  if(fabs(expArg) > 690)
    {
      ...
      // This next little section is very (i think) dumb. This is to correct a 
      // code crash (on QSC) when somehow, a value smaller than 1e-308 was 
      // assigned to X.k (but somehow not caught by the X.k == 0 check above)
      // which would crash the evaluation of log(X.k). I don't understand how
      // this is possible... but this fixes it.*/
      if(fabs(X.k) < 1e-300)
	{
	  dumb = 1e100 * X.k;
